...
RELATION_REGISTRY::FIGHT_DATA* RELATION_REGISTRY::FindFight(u16 object_id, bool by_attacker, u16 other_id)
{
	FIGHT_VECTOR& fights = fight_registry();
	for(FIGHT_VECTOR_IT it = fights.begin(); it != fights.end(); it++)
	{
		FIGHT_DATA& fight_data = *it;
		u16 id_to_find1 = by_attacker?fight_data.attacker:fight_data.defender;
		u16 id_to_find2 = by_attacker?fight_data.defender:fight_data.attacker;
		if(object_id == id_to_find1 && other_id == id_to_find2)
		{
			return &fight_data;
		}
	}
