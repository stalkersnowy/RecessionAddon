...
void CPolterBoth::LoadFlame(LPCSTR section)
{
	::Sound->create				(m_flame_sound,	pSettings->r_string(section,"flame_sound"), st_Effect, SOUND_TYPE_WORLD);
...
void CPolterBoth::FireFlame(const CObject *target_object)
{
	Fvector position;
	...
	m_object->PlayParticles(m_flame_particles_prepare,element->position,element->target_dir,TRUE);
}


struct remove_predicate_old {
	bool	operator() (CPolterBoth::SFlameElement *element)	{
...
void CPolterBoth::UpdateFlame()
{
	...
	for ( ;I != E; ++I) {
		...
		if (element->time_started + m_flame_fire_delay < time()) {
			...
			m_object->PlayParticles(m_flame_particles_fire,element->position,element->target_dir,TRUE);
	...
	// удалить все элементы, выполнение которых закончено
	I = std::remove_if(m_flames.begin(), m_flames.end(), remove_predicate_old());
	m_flames.erase(I,m_flames.end());
}

bool CPolterBoth::GetValidFlamePosition(const CObject *target_object, Fvector &res_pos)
{
	const CGameObject *Obj = smart_cast<const CGameObject *>(target_object);
	if (!Obj) return (false);
