...
void CTushkano::Load(LPCSTR section)
{
	...
	if(old_mutants) anim().accel_chain_add		(eAnimWalkDamaged,	eAnimRunDamaged);
	...
	anim().AddAnim(eAnimRunDamaged,		"stand_ran_dmg_",		-1,	&velocity_run_dmg,	PS_STAND);
	...
	anim().AddAnim(eAnimSitIdle,			"sleep_idle_",			-1, &velocity_none,				PS_SIT);
	anim().AddAnim(eAnimSitStandUp,		"sleep_stand_up_",		-1, &velocity_none,				PS_SIT);
	anim().AddAnim(eAnimStandSitDown,	"stand_to_sleep_",		-1, &velocity_none,				PS_STAND);
	...
	anim().LinkAction(ACT_SLEEP,			sit_anim);
