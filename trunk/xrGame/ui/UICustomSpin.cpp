...
void CUICustomSpin::Init(float x, float y, float width, float height){
	...
	float btn_size = BTN_SIZE * UI()->get_current_kx();
	float btn_size_m = (BTN_SIZE + 1) * UI()->get_current_kx();
	m_pBtnUp->Init				(width - btn_size_m, 0, btn_size, BTN_SIZE);
	m_pBtnUp->InitTexture		("ui_spiner_button_t");
	m_pBtnDown->Init			(width - btn_size_m, BTN_SIZE, btn_size, BTN_SIZE);
