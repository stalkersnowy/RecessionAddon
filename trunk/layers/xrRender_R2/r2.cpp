	...
	void*&							result)
{
	...
	// options
	{
		sprintf						(c_smapsize,"%04d",u32(o.smapsize));
	...
	if (ps_BloomMode)
	{
		...
		def_it						++;
		sh_name[len]='0'+char(ps_BloomMode); ++len;
	}
	else
	{
		sh_name[len]='0'; ++len;
	...
	if (ps_r2_ls_flags.test(R2FLAG_INDOOR))
	{
		defines[def_it].Name		=	"USE_INDOOR";
		defines[def_it].Definition	=	"1";
		def_it						++;
		sh_name[len]='1'; ++len;
	}
	...
	if (ps_r2_ls_flags.test(R2FLAG_HEMI_CUBE))
	{
		defines[def_it].Name		=	"HEMI_CUBE";
		defines[def_it].Definition	=	"1";
		def_it						++;
		sh_name[len]='1'; ++len;
	}
