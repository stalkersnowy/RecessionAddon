...
#include "../xrLC/xrDO_Light/R_light.h"
#include "light_db.h"
			...
			if (chunk)
			{
				...
				for (u32 i=0; i<count; i++) 
				{
					...
					//if (Ldata.type!=0)
					{
						...
						L->set_active		(true				);
						float falloff = 1.0f/(Ldata.range*(Ldata.attenuation0 + Ldata.attenuation1*Ldata.range + Ldata.attenuation2*Ldata.range2)); //SNW: в “„шный файл falloff ещЄ не сохран€лс€
						L->set_attenuation_params(Ldata.attenuation0, Ldata.attenuation1, Ldata.attenuation2, falloff);
...
void			CLight_DB::Update			()
{
	...
	if (sun_original && sun_adapted)
	{
		...
		if (!::Render->is_sun_static() && ps_r2_ls_flags.test(R2FLAG_TRUE_SUN))
		{
